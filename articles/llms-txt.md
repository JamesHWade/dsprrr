...
As pipelines grow, plain lists show their limits:

- No validation (what if a required field is missing?)
- No documentation (what fields does each result have?)
- Hard to compose (passing results between stages is error-prone)

S7 gives you typed containers:
...
Each stage returns a typed S7 object:
