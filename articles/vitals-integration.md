...
## Call a Module from a Function

Wrap [`run()`](https://jameshwade.github.io/dsprrr/reference/run.md)
when an application benefits from a task-specific function:

``` r

# Capture one configured Chat and keep dsprrr's typed execution boundary
chat <- chat_openai()
classify <- function(input) {
  run(sentiment_module, input = input, .llm = chat)
}

# Use like any R function
result <- classify("Amazing!")
...
dataset |>
  mutate(
    sentiment = map_chr(text, ~ classify(.x)$sentiment)
  )
```
...
# Explicit concurrent processing with progress bar
results <- sentiment_module |>
  run_dataset(
    large_dataset,
    .llm = chat_openai(),
