  ...
  return (
    <div className="space-y-4 lg:space-y-6">
      ...
      <div className="relative">
        ...
        <div className="hidden lg:block absolute left-[18px] top-0 bottom-0 w-px bg-border" />

        <div className="space-y-4 lg:space-y-6">
          {visibleIterations.map((iter, idx) => (
            <div key={iter.iteration} className="relative lg:pl-12">
              {/* Timeline dot */}
              <div className="hidden lg:block absolute left-[12px] top-6 w-3 h-3 rounded-full border-2 border-background bg-primary z-10" />
              <IterationCard
                iteration={iter}
      ...
      {currentIndex >= iterations.length - 1 && iterations.length > 0 && (
        <div className="text-center py-4">
          <span className="inline-flex items-center gap-2 rounded-full border border-green-200 dark:border-green-800 bg-green-50/60 dark:bg-green-950/30 px-4 py-2 text-sm font-medium text-green-700 dark:text-green-400">
