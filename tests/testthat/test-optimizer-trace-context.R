...
test_that("old trial records and checkpoints default trace context safely", {
  legacy_line <- paste0(
    '{"trial_id":"legacy-trace","optimizer_name":"TraceOptimizer",',
    '"params":[],"metric_summary":{"mean_score":0.3333},',
    '"cost_summary":[],"start_time":{},"end_time":{},',
    '"notes":"","status":"pending"}'
  )
  path <- file.path(withr::local_tempdir(), "legacy-trials.jsonl")
  writeLines(legacy_line, path)
  Sys.chmod(path, mode = "0600")
  restored <- read_trials_jsonl(path)[[1L]]
  expect_identical(restored@trace_context, list())
  write_trials_jsonl(list(restored), path)
  expect_identical(readLines(path, warn = FALSE), legacy_line)
...
test_that("Trial positional status remains backward compatible", {
  trial <- Trial(
    "positional-trial",
    "TraceOptimizer",
    list(),
  ...
  expect_identical(trial@status, "completed")
  expect_identical(trial@trace_context, list())
})
