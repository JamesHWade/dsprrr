...
Use `with_inputs()` to add constants that don\'t come from upstream outputs:
...
- When column names in your data frame don\'t match the pipeline inputs, rename
  them first or wrap the pipeline with `map_inputs()` to align fields.
