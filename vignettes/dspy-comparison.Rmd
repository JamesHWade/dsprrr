...
| Experimental `dspy.Flex` | `flex()` | Constrained analogue: a versioned JSON structural IR with allowlisted Predict and Chain-of-Thought steps. It does not evaluate arbitrary R/Python source and does not claim full DSPy Flex parity |
...
| `GEPA` | `GEPA` | Adapted ("GEPA-lite"): reflective mutation + whole-program Pareto selection; supports feedback metrics and complete validated Flex instruction/source components; no independent per-component frontier or inference-time search yet |
...
| Async | `acall`/`aforward`, `asyncify` | `run_async()` with promises for ordinary Predict modules; modules and composites with specialized `forward()` semantics are rejected rather than bypassed |
...
1. **Full Flex parity**: dsprrr's experimental `flex()` intentionally accepts
   only a validated JSON IR with Predict and Chain-of-Thought steps. It does
   not execute optimizer-authored R/Python or expose DSPy's arbitrary module
   source surface. GEPA-lite can search complete validated `module_src`
   components, but ranks whole-program candidates and has no inference-time
   search.
...
4. **Native reasoning-trace capture** as a typed output (analogous to
   `dspy.Reasoning`).
5. **Joint multi-step optimization for instruction optimizers** (MIPROv2,
