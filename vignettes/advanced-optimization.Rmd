...
| Combine multiple compiled programs | `ensemble()` | Low |
...
- `prompt_model`: Optional ellmer Chat for generating instructions; `NULL` uses
  the task Chat supplied through `.llm`
...
- `task_model`: Optional ellmer Chat for task evaluation; defaults to `.llm`
...
- `prompt_model`: Optional ellmer Chat for reflection; `NULL` uses SIMBA's
...
Reflective instruction and Flex-source optimization with failure feedback,
validation-example winner frontiers, and Pareto selection for multiple
objectives. dsprrr's adapted GEPA uses a fixed population/generations loop; it
does not expose arbitrary package state or unrestricted code execution.
...
provides discovery failures and reflection feedback, while `valset` supplies
aggregate selection scores, validation-example winners, and retained best
outputs. Pareto parent selection preserves both validation-example winners and
...
- `component_selector`: `"round_robin"`, budget-atomic `"all"`, or a custom
