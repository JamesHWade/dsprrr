...
The new `optimize_grid()` helper lets you tune a module directly without
going through a teleprompter first. It evaluates a grid of configuration
values (defined either as a simple named list or via tidymodels `dials`
parameters) and records the results in `module$state$trials`.
...
If you do have tidymodels installed, you can also describe the search space
with `dials`, build a grid (regular or random), and pass it to
`optimize_grid()`. The helper automatically captures the candidate
grid, scores, and metadata so teleprompters and downstream reports can reuse
the trial data.

### Registering parameter metadata

The `module_register_parameters()` helper lets you describe tunable fields once
and reuse the metadata whenever a parameter set is needed. This is especially
useful when you have not yet collected optimisation trials:
...
module_parameter_set(sentiment_module)
```

### Yardstick metrics & rsample resampling

`as_dsprrr_metric()` now understands yardstick metric sets and will return a
metric specification that produces tidy summaries. Combine this with
`rsample::vfold_cv()` (or any other `rset`) to evaluate each configuration on
multiple folds:
...
```{r evaluation}
#| cassette: compilation-evaluation
...
```{r custom-teleprompter, eval=FALSE}
