...
if (should_eval) {
  ...
  # Match stable transport identity rather than prompt bodies. Optimizer
  # internals may change request batching without changing the example's
  # documented behavior, and the cassette supplies responses in call order.
  vcr::vcr_configure(
    dir = "_vcr",
...
dsprrr provides three key helpers for integrating with the tidymodels ecosystem:

### `module_parameters()`: Derive dials parameters

After running `optimize_grid()`, you can extract a `dials::parameters()` set
from the module. This is useful for further tuning or for generating additional
grid configurations:

```{r parameter-set, eval = FALSE}
library(dials)
...
The function automatically discovers parameters from:
- Module configuration (e.g., `temperature`, `top_p`)
- Previous optimization trials
- Signature enum types (prefixed with `input_`)
- Known LLM parameter defaults

You can filter which parameters are included:

```{r parameter-set-filter, eval = FALSE}
...
For more detailed per-trial analysis, including integration with yardstick
metrics:

```{r metric-summary, eval = FALSE}
...
```{r tidymodels-workflow, eval = FALSE}
library(dials)
library(yardstick)
library(ggplot2)
