...
```{r}
#| include: false
# Skip evaluation during R CMD check, CI, or pkgdown (cassettes may not match current code)
# VITALS_SHOULD_EVAL can override for cassette recording
force_eval <- identical(Sys.getenv("VITALS_SHOULD_EVAL"), "true")

# Detect CI/pkgdown environment
...
# Skip eval in CI unless explicitly enabled (cassettes frequently mismatch)
should_eval <- force_eval || (!in_ci && !in_check && dsprrr:::eval_vignette())
...
if (should_eval) {
  # Disable dsprrr cache to ensure consistent HTTP recordings
  dsprrr:::configure_cache(enable_memory = FALSE, enable_disk = FALSE)
