...
Each module wraps a signature. We add `chain_of_thought()` to the analysis stages—this asks the LLM to show its work, which improves accuracy for complex analysis tasks. The final `llmstxt` stage just needs to synthesize; no reasoning required.

```{r simple-modules}
create_modules <- function(llm) {
  list(
    purpose = module(analyze_purpose_sig |> chain_of_thought()),
    structure = module(analyze_structure_sig |> chain_of_thought()),
    examples = module(generate_examples_sig |> chain_of_thought()),
