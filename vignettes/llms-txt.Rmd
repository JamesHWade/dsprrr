...
**Note:** This tutorial uses `eval = FALSE` for code chunks since it requires API access. The patterns shown are production-ready—copy them into your own scripts and supply your LLM.
...
Each module wraps a signature. We add `with_reasoning()` to the analysis stages—this asks the LLM to show its work, which improves accuracy for complex analysis tasks. The final `llmstxt` stage just needs to synthesize; no reasoning required.

```{r simple-modules}
create_modules <- function(llm) {
  list(
    purpose = module(analyze_purpose_sig |> with_reasoning()),
    structure = module(analyze_structure_sig |> with_reasoning()),
    examples = module(generate_examples_sig |> with_reasoning()),
