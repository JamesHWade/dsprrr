...
| `metric_fuzzy_match(threshold)` | Similarity above threshold |
| `metric_contains()` | Output contains expected |
| `metric_regex(pattern)` | Matches regex pattern |
| `metric_numeric_tolerance(tol)` | Numeric within tolerance |
| `metric_classification()` | Multi-class accuracy |
...
│
├─ Approximate match → metric_fuzzy_match(0.8)
│   └─ Paraphrased answers, spelling tolerance
│
├─ Contains key info → metric_contains()
│   └─ Important terms must appear
│
├─ Numeric answer → metric_numeric_tolerance(0.01)
│   └─ Math, measurements, scores
│
└─ Classification → metric_classification()
    └─ Sentiment, categories, labels
```
