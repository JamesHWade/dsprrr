...
writer is trusted; writable shared RDS caches can be poisoned. Legacy
project-local caches are not migrated automatically and should be removed or
...
| `"flex"` | `FlexModule` | Validated structural JSON IR (experimental) |
...
never closes a directly supplied runner; persistence and reset support are
backend-specific.
...
Flex version 1 permits only Predict and Chain-of-Thought steps. It never
evaluates arbitrary R/Python source and does not claim full DSPy Flex parity.
GEPA-lite validates complete candidate sources and ranks whole programs; it
does not reproduce DSPy's per-component frontiers or inference-time search.
