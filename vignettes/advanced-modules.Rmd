...
closes. Whether execution state persists depends on the backend. Serialize a
...
per invocation, owns the fresh runner, and closes it exactly once when the
...
`flex()` makes a bounded predictor topology part of a module's optimization
state. Its `module_src` is canonical, versioned JSON with only Predict and
Chain-of-Thought steps; it is data, never arbitrary R or Python source.
...
transactionally. Direct assignment to `module_src` is rejected. This safe IR
is intentionally narrower than DSPy's experimental Flex module and is not a
claim of full Flex parity. See [Structural Optimization with
Flex](flex-optimization.html) for the schema, references, and safety boundary.
...
| `flex()` | Testing bounded predictor structures | Experimental JSON schema |
