...
BestOfN tracks useful metadata:

```{r bon-metadata, eval=FALSE
result <- run(wrapper, question = "Test", .llm = llm)

# Access via .metadata
result$.metadata[[1]]$n_attempts     # How many attempts were made
result$.metadata[[1]]$best_score     # Score of selected result
result$.metadata[[1]]$all_scores     # Scores of all attempts
result$.metadata[[1]]$early_stopped  # Did we hit threshold?
result$.metadata[[1]]$total_tokens   # Tokens across all attempts
result$.metadata[[1]]$total_cost     # Cost across all attempts
```
...
1. **`chain_of_thought()`** - Force step-by-step reasoning
2. **`best_of_n()`** - Multiple attempts with scoring
3. **`refine()`** - Iterative improvement with feedback
4. **`multi_chain_comparison()`** - Ensemble synthesis

Start with ChainOfThought for complex tasks, add BestOfN for reliability, and graduate to Refine or MCC when you need maximum quality.
