---
title: "Advanced ellmer Integration"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Advanced ellmer Integration}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  eval = dsprrr::eval_vignette()
)
```

```{r setup}
library(dsprrr)
library(ellmer)
```

## Introduction

dsprrr is built on top of ellmer, Anthropic's R package for LLM interactions. This vignette explores advanced integration patterns that leverage ellmer's full capabilities.

## Parallel Processing

dsprrr supports two parallel processing methods for batch operations:

### Method 1: mirai (Default)

The default parallel method uses mirai for multi-process parallelism:

```{r parallel-mirai, eval = FALSE}
mod <- module(signature("text -> sentiment"), type = "predict")

# Process multiple items in parallel using mirai
results <- run(
  mod,
  text = c("I love this!", "This is terrible", "It's okay"),
  .parallel = TRUE,
  .parallel_method = "mirai"  # Default
)
```

### Method 2: ellmer Native

For more efficient parallelism, use ellmer's native `parallel_chat_structured()`:

```{r parallel-ellmer, eval = FALSE}
results <- run(
  mod,
  text = c("I love this!", "This is terrible", "It's okay"),
  .parallel = TRUE,
  .parallel_method = "ellmer"  # Uses ellmer's parallel HTTP requests
)
```

The ellmer method is more efficient because:
- Single process (no R subprocess overhead)
- Native async HTTP requests
- Better error handling
- Automatic rate limit handling

## Converting Modules to ellmer Tools

dsprrr modules can be converted to ellmer tools for use in agentic workflows:

```{r as-ellmer-tool, eval = FALSE}
# Create a sentiment analysis module
sentiment_mod <- module(
  signature("text -> sentiment: enum('positive', 'negative', 'neutral')"),
  type = "predict"
)

# Convert to an ellmer tool
sentiment_tool <- as_ellmer_tool(
  sentiment_mod,
  name = "analyze_sentiment",
  description = "Analyze the sentiment of text"
)

# The tool can now be registered with any Chat
chat <- chat_openai()
chat$register_tool(sentiment_tool)

# The LLM can now use the sentiment tool
chat$chat("Analyze the sentiment of: 'I love this product!'")
```

### Registering Tools Directly

For convenience, use `register_dsprrr_tool()` to create and register in one step:

```{r register-tool, eval = FALSE}
chat <- chat_openai()

# Create module
qa_mod <- module(
  signature("question -> answer", instructions = "Answer factual questions"),
  type = "predict"
)

# Register directly
register_dsprrr_tool(chat, qa_mod, name = "knowledge_lookup")

# Use the tool
chat$chat("Use knowledge_lookup to find: What is the capital of France?")
```

## Leveraging ellmer's Cost Tracking

ellmer provides robust token and cost tracking. dsprrr integrates with this:

```{r cost-tracking, eval = FALSE}
# After running some predictions
result <- dsp("question -> answer", question = "What is 2+2?")

# Get the last trace which includes cost info
trace <- get_last_trace()
trace$assistant_turn@tokens  # [input_tokens, output_tokens, cached_tokens]
trace$assistant_turn@cost    # Cost in dollars

# For batch operations, use session_cost()
session_cost()
```

### Token Usage from Chat Objects

```{r token-usage, eval = FALSE}
chat <- chat_openai()

# Run some predictions
result <- chat |> dsp("q -> a", q = "Hello")

# Get token usage from the Chat
chat$token_usage()
```

## Advanced Chat Patterns

### Using Chat Objects Across Multiple Calls

```{r chat-reuse, eval = FALSE}
# Create a Chat and reuse it
chat <- chat_openai()

# First call - establishes conversation
result1 <- chat |> dsp("q -> a", q = "What is R?")

# Second call - same Chat, conversation continues
result2 <- chat |> dsp("q -> a", q = "What about Python?")

# The Chat remembers context from previous calls
```

### Default Chat Management

```{r default-chat, eval = FALSE}
# Set a default Chat for all dsp() calls
set_default_chat(chat_openai(model = "gpt-4o"))

# Now dsp() uses the default
result <- dsp("q -> a", q = "What is 2+2?")

# Check current configuration
dsprrr_sitrep()

# Clear when done
clear_default_chat()
```

## Multimodal Support

dsprrr inherits ellmer's multimodal capabilities:
```{r multimodal, eval = FALSE}
mod <- module(
  signature("image, question -> answer"),
  type = "predict"
)

# Pass an image via ellmer Content objects
result <- run(
  mod,
  image = ellmer::ContentImageRemote("https://example.com/image.jpg"),
  question = "What is in this image?"
)
```

## Streaming Responses

For long-form generation, use streaming:

```{r streaming, eval = FALSE}
mod <- module(
  signature("topic -> essay"),
  type = "predict"
)

# Stream with callback
mod$stream(
  list(topic = "The future of AI"),
  callback = function(chunk) cat(chunk)
)

# Or use async streaming
stream <- stream_async(mod, topic = "The future of AI")
```

## Error Handling and Retries

ellmer handles retries automatically. Configure via options:

```{r retries, eval = FALSE}
# Set max retries (default is 3)
options(ellmer_max_tries = 5)

# Set timeout (default is 60 seconds)
options(ellmer_timeout = 120)

# dsprrr wraps errors with helpful context
tryCatch(
  dsp("q -> a", q = "test"),
  error = function(e) {
    # Error includes model info, suggestions, etc.
    print(e)
  }
)
```

## Summary

Key integration points with ellmer:

1. **Parallel processing**: Choose between mirai (multi-process) or ellmer native (async HTTP)
2. **Tool integration**: Convert modules to ellmer tools with `as_ellmer_tool()`
3. **Cost tracking**: Leverage ellmer's `token_usage()` and dsprrr's `session_cost()`
4. **Chat management**: Use `set_default_chat()` for session-wide defaults
5. **Multimodal**: Pass ellmer Content objects for images, PDFs, etc.
6. **Streaming**: Use `stream()` or `stream_async()` for long-form generation
7. **Error handling**: Benefit from ellmer's automatic retries and dsprrr's context-rich errors
