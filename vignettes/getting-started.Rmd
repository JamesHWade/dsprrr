...
## What is dsprrr?
dsprrr brings [DSPy](https://github.com/stanfordnlp/dspy) to R. Instead of tweaking prompt strings, you declare what you want and let the framework handle how to ask for it.
...
If you use ellmer, this will feel familiar. `dsp()` is like `chat$chat_structured()` but with signatures:

```{r dsp-basic, eval=FALSE}
chat <- chat_openai()
...
```{r signatures-explicit, eval=FALSE}
sig <- signature(
  inputs = list(
...
- `vignette("compilation-optimization")` — Deep dive into optimization
- `vignette("vitals-integration")` — Evaluation with vitals
- `vignette("orchestration")` — Production workflows
