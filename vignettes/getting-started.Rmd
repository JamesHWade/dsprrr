...
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  eval = FALSE # Disable evaluation for now to debug
)
...
# Alternative: Direct constructor access (for advanced users)
# Note: Constructors require structured arguments, not strings
sig <- Signature(
  inputs = list(input("text")),
  output_type = ellmer::type_string()
)
result2 <- Predict(signature = sig, template = "Summarize: {text}") |>
  run(
    text = "The tidyverse is a collection of R packages designed for data science. All packages share an underlying design philosophy, grammar, and data structures.",
    .llm = llm
  )
...
# Or use constructors directly: Signature(), Predict(), run()
...
info <- Signature(
  ...
  output_type = type_object(
    ...
    entities = type_array(
      type_object(
        name = type_string(),
        type = type_enum(c("person", "organization", "location", "product"))
      )
    ),
    key_facts = type_array(type_string()),
    summary = type_string(max_length = 100)
  ),
...
summarizer <- Signature(
  inputs = list(input("text")),
  output_type = type_object(
    summary = type_string(max_length = 200),
...
inp7 <- input("status", type_enum(c("pending", "done")))
  ...
  output_type = type_object(
    ...
    sections = type_array(
      type_object(
        title = type_string(),
        content = type_string(),
        priority = type_enum(c("high", "medium", "low"))
      )
    ),
...
analyze_customer_feedback <- function(feedback, llm = NULL) {
  feedback |>
    # Step 1: Extract sentiment and key issues
    (\(text) {
      signature("feedback -> sentiment, issues: array(string)") |>
        module(type = "predict") |>
        run(feedback = text, .llm = llm)
    })() |>
    # Step 2: Generate response based on sentiment
    (\(analysis) {
      signature("sentiment, issues -> response") |>
        module(
          type = "predict",
...
# Use it
response <- analyze_customer_feedback(
...
modules <- list(
  summarize = signature("text -> summary: string[50, 200]") |>
    module(type = "predict"),
  sentiment = signature(
    "text -> sentiment: enum('positive', 'negative', 'neutral')"
  ) |>
...
good_sig <- Signature(
  ...
  output_type = type_object(
    category = type_enum(c("complaint", "question", "feedback", "request")),
...
While this vignette covers the foundational features currently available in dsprrr, the package roadmap includes exciting upcoming features:

1. **Optimization**: Automatic prompt optimization using techniques like bootstrap few-shot learning
2. **Advanced Modules**: ChainOfThought, ReAct agents, and other reasoning strategies
3. **Evaluation Framework**: Built-in metrics and evaluation pipelines
4. **Caching and Persistence**: Automatic caching of LLM calls and results
5. **Multi-model Support**: Easy switching between different LLM providers
