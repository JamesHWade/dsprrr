...
**dsprrr** is an R package that brings the power of DSPy (Declarative Self-improving Language Programs) to the R ecosystem. It provides a principled framework for building AI applications using Large Language Models (LLMs), moving beyond brittle prompt strings to structured, composable, and optimizable programs.

Instead of wrestling with prompt engineering, dsprrr lets you:

- **Declare** what you want your LLM to do using expressive DSPy-style signatures
- **Execute** your AI logic through modular `Predict` components
- **Compose** multiple modules into sophisticated pipelines
- **Evaluate** your programs systematically - see [Evaluation and Metrics](evaluation-metrics.html)
- **Optimize** your programs automatically - see [Automatic Prompt Optimization](compilation-optimization.html)
...
Here's the simplest way to get started with dsprrr using R's pipe operator:

```{r quickstart}
#| cassette: getting-started-quickstart
...
That's it! No complex setup, just simple and expressive code that flows naturally with R's pipe operator (`|>`).

### Flexible API Design

dsprrr provides both user-friendly functions and direct constructor access:

```{r api-styles}
#| cassette: getting-started-api-styles
...
The combination of pipes and consistent naming makes dsprrr feel truly native to R:

```{r why-consistency}
#| cassette: getting-started-why-consistency
...
A `Signature` defines the schema for an LLM operation - what inputs it takes, what output it produces, and optional instructions for the task.

dsprrr supports two ways to create signatures:

1. **String notation** (recommended for simplicity): DSPy-style compact strings like `"text -> sentiment"`
2. **Explicit notation** (for complex cases): Full control over input/output specifications

#### Unified Type System

dsprrr uses a consistent type system based on ellmer for both inputs and outputs. You no longer need to know about S7 classes!

**For inputs**, you can use:
- Simple strings: `"string"`, `"number"`, `"boolean"`, `"integer"`
- Default behavior: Omit type and it defaults to string
- Ellmer types: `type_string()`, `type_number()`, etc. for advanced needs

**For outputs**, you have:
- String notation in signatures: `"-> answer: string"`, `"-> score: number[0,100]"`
- Ellmer types for complex structures: `type_object(...)`, `type_array(...)`
- Enums: `type_enum(values = c("a", "b", "c"))`
...
A `Predict` module pairs a signature with an optional template to create an executable unit. With pipes, this becomes beautifully concise:

```{r predict-basic}
#| cassette: getting-started-predict-basic
...
Let's build a more sophisticated classifier that returns both a label and confidence score:

```{r classification-advanced}
#| cassette: getting-started-classification-advanced
...
```{r extraction}
#| cassette: getting-started-extraction
...
```{r few-shot}
#| cassette: getting-started-few-shot
...
```{r pipeline}
#| cassette: getting-started-pipeline
...
dsprrr provides flexible ways to specify input types - no more S7 classes required!
...
The pipe operator (`|>`) isn't just syntactic sugar in dsprrr—it fundamentally makes LLM programming feel like natural R:
...
```{r complex-workflows, eval = eval_vignette()}
...
dsprrr supports advanced type annotations including Optional, Union, and dict types:

```{r complex-types, eval=FALSE}
...
For detailed field descriptions, use ellmer types directly with their `description` parameter:

```{r field-descriptions, eval=FALSE}
...
```{r best-practices-templates, eval = eval_vignette()}
...
```{r best-practices-demos, eval = eval_vignette()}
...
```{r best-practices-modular, eval = eval_vignette()}
...
```{r debugging, eval = eval_vignette()}
...
Now that you understand the basics, dive deeper into the core workflows:

### Learn the Complete Development Cycle

1. **[Evaluation and Metrics](evaluation-metrics.html)** - Learn how to systematically evaluate your LLM programs
   - Define metrics that measure what matters
   - Run evaluations on test datasets
   - Use tidymodels integration (yardstick, rsample)

2. **[Automatic Prompt Optimization](compilation-optimization.html)** - Optimize your programs systematically
   - Use `optimize_grid()` for direct grid search
   - Apply teleprompters (LabeledFewShot, GridSearchTeleprompter)
   - Leverage tidymodels for cross-validation

3. **[Integration with vitals](vitals-integration.html)** - Advanced evaluation workflows
   - Convert modules to vitals solvers
   - Use vitals scorers in optimization
   - Complete end-to-end workflows

### Upcoming Features

The package roadmap includes:

- **Advanced Modules**: ChainOfThought, ReAct agents, and other reasoning strategies
- **Caching and Persistence**: Automatic caching of LLM calls and results
...
- [DSPy Paper](https://arxiv.org/abs/2310.03714) - The original DSPy framework
- [ellmer Package](https://github.com/hadley/ellmer) - LLM provider integration

## Summary

dsprrr brings the elegance of DSPy to R with:

- **Expressive syntax**: Write `signature("text -> sentiment")` for clear, concise specifications
- **Simple input types**: Use `input("text")` or `input("count", "integer")` for straightforward type definitions
- **Flexible approaches**: Use string notation for simple cases, explicit notation for complex ones
- **Consistent type system**: Both inputs and outputs use the same ellmer-based types

## Getting Help

If you encounter issues or have questions:

1. Check the package documentation: `?dsprrr`
2. File an issue on [GitHub](https://github.com/jameshwade/dsprrr/issues)
3. See more examples in other vignettes

Remember: dsprrr helps you write **programs**, not prompts. Think about your LLM operations as composable functions with well-defined inputs and outputs, and let the framework handle the complexity of prompt engineering.
