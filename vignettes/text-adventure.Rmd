...
#| results: asis
cat("
> **Note**: Code in this tutorial requires an OpenAI API key to run. Set `eval = TRUE` and configure your API credentials to execute interactively. The code is tested and functional—`eval = FALSE` keeps the vignette buildable without API access.
")
```
...
Each signature becomes a module. We wrap them with `with_reasoning()` which adds a "reasoning" field to the output—the model explains its thinking before giving the final answer. This chain-of-thought approach improves output quality, especially for complex decisions like whether an action should succeed.
