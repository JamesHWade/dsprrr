...
process/protocol failure. A successful result may omit `error` and
`error_type`.
...
idempotent terminal `shutdown()`. dsprrr starts a factory runner before
use and shuts it down exactly once on success, error, or interrupt.
