...
For tasks requiring exact computation, dsprrr can generate and execute R code:

```{r code-execution, eval=FALSE}
...
**Security**: Code execution uses subprocess isolation via `callr` with timeouts, output limits, and dangerous pattern blocking. For production with untrusted inputs, use OS-level sandboxing.
...
- **Code Execution**: ProgramOfThought and CodeAct modules with safe R code execution
- **Advanced Reasoning**: ChainOfThought, BestOfN, Refine, MultiChainComparison
